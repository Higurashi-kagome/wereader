...
function getShelf(shelfData: ShelfDataTypeJson) {
    ...
    const bookIdBookMap = books!.reduce((tempMap: {[propName: string]: Book}, curBook: Book) => {
    ...
    const shelf = categoryObjs!.reduce((
        ...
        curCate.bookIds.forEach(bookId => {
            cate.books.push(bookIdBookMap[bookId])
            delete bookIdBookMap[bookId]
        })
        tempShelf.push(cate)
        return tempShelf
    }, [])
    ...
    Object.keys(bookIdBookMap).forEach((bookId) => {
        extraCate.books.push(bookIdBookMap[bookId])
    })
