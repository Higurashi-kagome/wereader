...
function initRegexp(setting: { [x: string]: unknown; re?: any; }) {
    ...
    const keys = Object.keys(replacePatterns)
    for (let i = 0; i < keys.length; i++) {
        ...
        checkBox.on('click', function () {
            const target = $(this)
            const rgxInput = target.parent().parent().find('.' + RegexpInputClassName)
            if (rgxInput.val() === '' && target.attr('checked')) { // 检查 this.checked 使得取消选中时不会触发
                rgxInput.attr('placeholder', '请输入匹配模式')
