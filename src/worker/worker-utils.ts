...
async function getUserVid(url: string = Wereader.maiUrl): Promise<string> {
...
/**
 * 发消息给 content 脚本，请求 wereader api 获取数据
 * @param wereader Wereader 实例，content 脚本将用于实例化 Wereader 对象
 * @param methodName wereader api 方法名
 * @param isStatic 是否静态方法
 * @param args wereader api 方法参数
 * @returns {Promise<unknown>} 请求响应
 */
async function requestContentWereader(
    wereader: Wereader,
    methodName: string,
    args: unknown[] = [],
...
// eslint-disable-next-line no-undef
async function getText(url: string, options: RequestInit = {
    credentials: 'include',
    cache: 'no-cache'
}) {
    try {
