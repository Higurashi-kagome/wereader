...
function insertHelpContent(){
    ...
    for (const id in helpContent) {
        ...
        if(helpContent[id].help){
            target.innerHTML = helpContent[id].title + `<span class="help-icon">${helpIcon}</span>`;
            let help = document.createElement('div');
            help.className = 'help-content';
            help.setAttribute('hidden', '');
            help.innerHTML = helpContent[id].help;
            insertAfter(help, target.parentNode);
        }else if(helpContent[id].title){
            target.innerHTML = helpContent[id].title;
        }
    }
}
